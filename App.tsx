...
import { Send, Flame, Zap, Crown, ArrowRight, Users, Globe } from 'lucide-react';
...
export default function App() {
  ...
  const handleCreatePost = async (e: React.FormEvent) => {
    ...
    await mockBackend.createPost(newPostContent, currentUser, isMiracleRequest, PromotionTier.NONE);
    setNewPostContent('');
    loadFeed();
  };
  ...
  const filteredPosts = posts.filter(post => {
    if (feedFilter === 'community' && currentUser) {
      // Mostrar si es de la misma religión O si está en su círculo
      return post.authorReligion === currentUser.religion || currentUser.circleIds?.includes(post.userId);
    }
    return true;
  });
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-[3.5rem] shadow-xl p-12 mb-12 border border-slate-50">
        <form onSubmit={handleCreatePost} className="space-y-8">
          <div className="flex space-x-6">
            <img src={currentUser.avatarUrl} alt="" className="h-16 w-16 rounded-2xl" />
            <textarea 
          ...
          <div className="flex items-center justify-between pt-8 border-t border-slate-50">
            <button type="button" onClick={() => setIsMiracleRequest(!isMiracleRequest)} className={`flex items-center space-x-3 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest ${isMiracleRequest ? 'bg-amber-400 text-white' : 'bg-slate-50 text-slate-400'}`}>
              <Zap className="h-4 w-4" /> <span>{t['miracle_call']}</span>
            </button>
            <Button type="submit" className="px-10 py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl shadow-indigo-100 bg-indigo-600">
      ...
      <div className="flex bg-slate-100 p-2 rounded-[2.5rem] mb-12 border border-slate-200">
        <button onClick={() => setFeedFilter('community')} className={`flex-1 flex items-center justify-center py-5 rounded-[2rem] text-[10px] font-black uppercase tracking-widest transition-all ${feedFilter === 'community' ? 'bg-white text-indigo-700 shadow-xl' : 'text-slate-400'}`}>
          <Users className="h-4 w-4 mr-3" /> {t['my_circle']}
        </button>
        <button onClick={() => setFeedFilter('all')} className={`flex-1 flex items-center justify-center py-5 rounded-[2rem] text-[10px] font-black uppercase tracking-widest transition-all ${feedFilter === 'all' ? 'bg-white text-indigo-700 shadow-xl' : 'text-slate-400'}`}>
      ...
      {showTithingModal && <TithingModal user={currentUser} onClose={() => setShowTithingModal(false)} onSuccess={() => setShowTithingModal(false)} />}
